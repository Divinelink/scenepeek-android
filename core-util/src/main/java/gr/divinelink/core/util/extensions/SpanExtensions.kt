...
fun CharSequence?.bold(): CharSequence {
    ...
    spannable.setSpan(StyleSpan(Typeface.BOLD), 0, this?.length ?: 0, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
    return spannable
}

fun CharSequence?.textAppearance(context: Context, @StyleRes textAppearance: Int): CharSequence {
    val spannable = SpannableString(this)
    spannable.setSpan(TextAppearanceSpan(context, textAppearance), 0, this?.length ?: 0, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
    return spannable
}

fun CharSequence?.center(): CharSequence {
    val spannable = SpannableString(this)
    spannable.setSpan(AlignmentSpan.Standard(Layout.Alignment.ALIGN_CENTER), 0, this?.length ?: 0, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
    return spannable
}

fun CharSequence?.color(color: Int): CharSequence {
    val spannable = SpannableString(this)
    spannable.setSpan(ForegroundColorSpan(color), 0, this?.length ?: 0, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
    return spannable
}
